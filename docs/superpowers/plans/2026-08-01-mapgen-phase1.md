...
- Produces:
  ...
  - `check_path_length(paths: PackagePaths, overture_types: Sequence[str], limit: int = 240) -> None`, raising `PathTooLongError`. It measures the longest path the job will actually produce, which means both the nested tile path and `paths.project_setting`. The project setting file embeds the full stem and can be the longer of the two, so checking only the tile path lets real overruns through. The tile path includes the tiling fingerprint segment introduced in Task 13, so the worst case is `_work/<8 hex>/raw/overture/<longest type>/rNN_cNN.geojson`. Under-measuring by that segment is exactly the overrun this guard exists to prevent.
  - `NamingError(ValueError)` and `PathTooLongError(NamingError)`.

**Resume design, decided during Task 13 and recorded here because it constrains this module.** Tile ids are `r{row:02d}_c{col:02d}`, which identifies a position in a grid but says nothing about which grid. The grid is determined by bbox, tile size and overlap, so two different tilings produce colliding tile ids for different geography. Rather than detect that collision, each tiling gets its own work directory keyed by a fingerprint of those six values: `_work/<8 hex>/state.json` and `_work/<8 hex>/raw/<source>/...`. Identical parameters resume; any different parameters are structurally isolated and cannot see each other's files. This replaced three successive attempts at collision detection, all of which closed one path and left another open.
